...
3. `ledger.Ledger.Apply` executes the command against `store.SQLite` and returns a `ledger.Reply{Text, Ephemeral}`. Every successfully handled command gets a `:white_check_mark:` reaction; app mentions also keep the `:sergeant:` acknowledgement. Add/subtract/reset commands use reactions only, including in DMs: `:sergeant:` plus `:white_check_mark:` after success and `:sergeant-no:` after failure, with no text response. Other replies are posted through the `Responder` interface according to `ledger.Reply`.
