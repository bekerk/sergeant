...
3. `ledger.Ledger.Apply` executes the command against `store.SQLite` and returns a `ledger.Reply{Text, Ephemeral}`. The handler posts it via the `Responder` interface — in-thread for add/reset, ephemeral for status/pay/help.
